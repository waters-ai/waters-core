...
<div class="dash">
  ...
  <div class="chat-box" id="chat-box">
    ...
    <div class="chat-input">
      ...
      <button onclick="voiceInput()" id="voice-btn" title="Голосовой ввод (микрофон)" style="background:linear-gradient(135deg,#ff6b6b,#ee5a24);color:#fff;padding:10px 16px;border:0;border-radius:8px;font-weight:600;cursor:pointer;font-size:16px">🎤</button>
      <button onclick="toggleVoice()" id="voice-toggle" title="Озвучивать ответы" style="background:rgba(255,255,255,0.1);color:#fff;padding:10px;border:0;border-radius:8px;cursor:pointer;font-size:16px;opacity:0.5">🔇</button>
    </div>
  </div>
</div>
<script>
...
function speak(text){if(!voiceEnabled)return;synth.cancel();let u=new SpeechSynthesisUtterance(text);u.lang='ru-RU';u.rate=0.9;synth.speak(u)}
function startStream(){let sessionId='sess_'+Date.now();let btn=document.getElementById('send-btn');btn.disabled=true;btn.textContent='Stream...';let vb=document.getElementById('voice-btn');vb.disabled=true;document.getElementById('stream-status').textContent='🔴 streaming';if(evtSource)evtSource.close();evtSource=new EventSource('/api/v1/stream/'+sessionId);let box=document.getElementById('messages');let msgDiv=document.createElement('div');msgDiv.className='chat-msg';msgDiv.innerHTML='<span class=role>assistant: </span><span class=text id=stream-text></span>';let fullText='';box.appendChild(msgDiv);evtSource.onmessage=function(e){try{let d=JSON.parse(e.data);if(d.type==='done'){evtSource.close();evtSource=null;btn.disabled=false;btn.textContent='Send';vb.disabled=false;document.getElementById('stream-status').textContent='✅ done';speak(fullText);return}let el=document.getElementById('stream-text');if(d.type==='token'){if(el)el.textContent+=d.content;fullText+=d.content}else if(d.type==='reasoning'){let r=document.getElementById('stream-reasoning');if(!r){r=document.createElement('div');r.className='reasoning';r.id='stream-reasoning';msgDiv.appendChild(r)}r.textContent+=d.content}}catch(e){console.log(e)}};evtSource.onerror=function(){btn.disabled=false;btn.textContent='Send';vb.disabled=false;document.getElementById('stream-status').textContent='❌ error'}}
function voiceInput(){let SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SpeechRecognition){alert('Голосовой ввод не поддерживается в этом браузере. Используйте Chrome.');return}let recognition=new SpeechRecognition();recognition.lang='ru-RU';recognition.interimResults=false;recognition.continuous=false;let btn=document.getElementById('voice-btn');btn.textContent='🔴';btn.style.background='linear-gradient(135deg,#ff4757,#c0392b)';recognition.onresult=function(e){let text=e.results[0][0].transcript;document.getElementById('chat-input').value=text;btn.textContent='🎤';btn.style.background='linear-gradient(135deg,#ff6b6b,#ee5a24)';sendChat()};recognition.onerror=function(){btn.textContent='🎤';btn.style.background='linear-gradient(135deg,#ff6b6b,#ee5a24)'};recognition.start()}
function toggleVoice(){voiceEnabled=!voiceEnabled;let btn=document.getElementById('voice-toggle');if(btn){btn.textContent=voiceEnabled?'🔊':'🔇';btn.style.opacity=voiceEnabled?1:0.5}}
